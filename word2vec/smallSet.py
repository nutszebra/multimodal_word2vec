...
def swapKeyAndVal(dic):
  answer = {}
  for key in dic:
    if not dic[key] in answer:
      answer[dic[key]] = []
    answer[dic[key]].append(key)
  return answer

def swapValAndKey(dic):
  answer = {}
  for key in dic:
   for i in dic[key]:
     answer[i] = key
...
def copyFile(namelist, path_from, path_to):
  for name in namelist:
    shutil.copy2(path_from + name, path_to + name)
...
if __name__ == '__main__':
 # x_train, y_train, x_test, y_test, tag = cifar100Extract()
  picNum = 100
  trainPath = picBase + "label/y_train.pkl"
  train_y = load_object(trainPath)
  train_y_sw = swapKeyAndVal(train_y)
  newTrainSet = {}
  for key in train_y_sw:
    newTrainSet[key] = random.sample(train_y_sw[key], picNum)
  if not checkExistance(picBase + "smallTrain_x"):
    makeDirectory(picBase + "smallTrain_x")
    makeDirectory(picBase + "smallTrain_y")
    for key in newTrainSet:
      copyFile(newTrainSet[key], picBase + "train/", picBase + "smallTrain_x/")
    save_object(swapValAndKey(newTrainSet), picBase + "smallTrain_y/train_y.pkl")
